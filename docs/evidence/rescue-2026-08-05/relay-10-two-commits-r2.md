═══════════════════════════════════════════════════════════════════════════
  ...
  Rev 2 — three edits from the HEB-86 review surface, 2026-07-29.
═══════════════════════════════════════════════════════════════════════════
...
**Deliberately NOT in this list:** the `author-execution-relay` skill. That
skill is authoring-side only — its own Scope section states that executor
behavior is governed by the relay artifact it produces, "not by the
executor loading this skill." Your authority for this transaction is this
document plus the two doctrines above. The omission is deliberate, not an
oversight; do not go looking for it.
...
**Note on `<N>`, so a later reader does not score it as an oversight.**
This is a named fill-slot whose rule routes a contested figure to the only
surface that can derive it — you hold the per-step tables; the drafting
surface does not. That is a deliberate departure from the verbatim-payload
discipline, which would otherwise require a pinned post-fill byte string.
The compensating control is Gate C1-V: you report `git log -1 --format=%B`
verbatim, and Tad reads the actual message before the transaction advances.
...
content, not from the working tree, and state the unit on every figure:
  ...
  b. Piece 2's character count and its structured/prose split, using the
     ...
     item; prose otherwise; blank lines excluded.*

     **Boundary convention — state which one you used, and do not leave it
     to inference.** Piece 2 is delimited by the literal `\n## Piece 2` and
     `\n## Piece 3`. The canonical segment is **exclusive of both delimiter
     newlines**: it begins at the `#` of `## Piece 2` and ends at the last
     character before the `\n` that precedes `## Piece 3` — i.e. the slice
     `s[s.index('\n## Piece 2')+1 : s.index('\n## Piece 3')]`.

     Under that convention the expected figures are **41,006 characters /
     41,314 bytes**. Including the leading delimiter newline yields 41,007
     / 41,315 — a one-character difference that has already caused one
     divergence between two surfaces. Report the convention alongside the
     number; a figure without its predicate is not a measurement.

     The structured/prose split is unaffected by the convention, because
     blank lines are excluded either way. Expected: structured 19,433
     characters (47.5%), prose 21,503 characters (52.5%).

**Derive all of the above yourself before reading the expected figures, and
report any difference rather than reconciling to them.**
