...
description: Author a construct spec — the engineering design document for a change to one construct (a runner, harness, instrument, pipeline, or a service's internal machinery), whose consumer is an implementer who must build it from the document alone. Carries the two-jobs discipline that keeps the ratification record from migrating into the spec, the rebuild-test floor, the shape (document frame, the piece spine, obligation sets), what the implementation target must declare, and what graduates into standing contract documents at acceptance. Use when writing or amending a construct spec, programming spec, implementation spec, or harness/instrument/runner design; when such a document is growing under an active build; when pulling deliberation or accumulated history back out of one; and when one reaches acceptance and its durable rules must move into standing contract documents. NOT for a product feature spec or PRD — a construct spec's consumer is an implementer building machinery, not a team scoping a feature. Assumes the three-actor working model (deliberation surface authors, executor builds, operator ratifies); not stack-bound — the target may be service code, a prompt artifact, a shell script, or infrastructure. Which doctype this is, and the body-integrity and amendment-lifecycle disciplines, belong to author-decision-record; authoring the reusable standard itself is author-standard.
---
