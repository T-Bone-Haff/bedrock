...
5. **No secrets in configuration or state.** Secrets come from Secret Manager via data sources, never hardcoded and never committed. Mark sensitive variables and outputs `sensitive = true` (knowing that still leaves them plaintext in state — see `reference/03-state-and-environments.md` §4).
