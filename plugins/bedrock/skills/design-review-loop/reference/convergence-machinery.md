...
- **`HALT_DECISION`** — surfaced to the operator, one finding at a time, unbundled. Its `reason` is one of three: an open, counted-severity `decision-bearing` finding (this takes precedence over convergence); **`oscillation`** — a closed finding reopened (recurrence: real two-altitude trading); or **`non-convergence`** — the open counted set stopped strictly decreasing with no recurrence (plateau: accumulation the operator must come rule). The oscillation/non-convergence split is deliberate — see the two facts below.
...
- **`classification`** — `unclassified` → `resolvable` | `decision-bearing`, set by the arbiter. A decision-bearing defect must use `BLOCKING` or `MATERIAL`; `decision-bearing` plus `COSMETIC` is an invalid no-action/action-required combination that aborts before routing and returns for severity correction.
...
- **Invalid severity/classification pairs fail loudly.** A runner without the `decision-bearing` plus `COSMETIC` rejection guard is not a conforming runner-backed profile and may not claim mechanical convergence.
