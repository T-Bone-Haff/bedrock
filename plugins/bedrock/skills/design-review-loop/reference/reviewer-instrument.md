...
5. **Severity is the house ladder.** `BLOCKING` (must resolve before the set can stand) / `MATERIAL` (should be fixed in scope; accumulating these is real debt) / `COSMETIC` (worth recording, not worth holding the set for) / `POSITIVE` (rule 7). Report your honest read of weight; convergence counting is not your concern. A defect that identifies an unresolved choice is at least `MATERIAL`, even though the arbiter—not the reviewer—owns the later `resolvable` versus `decision-bearing` classification. Never emit a choice requiring operator action as `COSMETIC`.
